...
- Step 14 (plan generation): `POST /investment-plans/{name}/actions/generate` answers `200 {name, count, transactions: [TransactionDetail], nextTransactionDate}`; a dry run `200 {name, dryRun, count, dates[]}`. `InvestmentPlan#getDatesOfTransactionsToBeGenerated()` is the extracted loop; `generateTransactions` iterates it. Because `generateTransactions` would keep the transactions it created before a missing price, the handler checks `Security#getSecurityPrice(date)` for every due date of a purchase plan first and answers `409 missing-price` (new problem type, instrument and date in `detail`) without generating anything. A generation that has nothing to do does not mark the file dirty. Logged as `MsgApiPlanTransactionsGenerated`. The core module's tests need `-o` dropped once to fetch the JaCoCo plugin.
- Step 15 (quote update job):
  - SPI: `void HostApplication#startPriceUpdate(OpenFile, List<Security>, Set<PriceUpdateTarget>, Consumer<IStatus> onDone)` (no return value; the rest plugin owns the job id). `RestApiAddon` runs `UpdatePricesJob` with an identity-set predicate, `suppressAuthenticationDialog(true)` and a `JobChangeAdapter`. A status of `ERROR` or `CANCEL` makes the job `failed` (`error` = message or `cancelled`), anything else `done`.
  - `JobRegistry` states are `running`, `done`, `failed` (no `pending`: the job is scheduled before the answer). A job is scoped to its file (`GET /jobs/{id}` of another file is `404`). Job JSON: `{jobId, kind: "update-quotes", status, startedAt, finishedAt?, error?, summary?}`; `summary` = `{instruments, latestUpdated, historicUpdated, updated[{uuid, name, latest, latestDate?, historicPricesAdded}], dirty}` from before/after snapshots of latest price, price count and last historical price, taken on the UI thread.
  - `POST /actions/update-quotes` is gated like a write (UI thread, `423`), then waits on the HTTP worker thread: `wait` is 0-60 seconds (anything else `400 invalid-value`). A dry run answers `200 {dryRun, instruments[{uuid, name}], targets[]}`. `instruments: []` and `targets: []` are `invalid-value` (omit them for "all"); an unknown UUID is `instruments[i]` `unknown-reference`. The completion is logged as `MsgApiPricesUpdated`.
- Step 16 (PDF import):
  - `POST /imports/pdf` `{paths[], targets?}` answers `200 {importId, kind: "pdf", expiresAt, items[], counts{ok, warning, error}, errors[{path, message}]}`. Paths must be absolute, end in `.pdf` (`invalid-value`) and be readable (new `FieldError` code `file-not-found`); at most 100. The route resolves the file and validates `targets` on the UI thread, extracts on the HTTP worker thread, then checks and registers the session on the UI thread; it is not gated by `423` (it changes nothing) and has no dry run.
  - The checks need targets (the wizard's page is the `ImportAction.Context`), so the preview accepts optional `targets` too (the adapter sends none) and otherwise uses the wizard's preselection: first cash account in the currency by name (active first), first active investment account, no default secondary investment account. The commit re-runs the checks with its own `targets`.
  - Item: `{index, kind, status, checks[{code, status, message?}], investmentPlanItem?, transaction | instrument (+ price), cashAccount/investmentAccount/fromCashAccount/toCashAccount/fromInvestmentAccount/toInvestmentAccount {uuid, name}}`. `kind`: `buy-sell`, `transaction`, `cash-transfer`, `security-transfer`, `instrument`, `instrument-update`, `price`, `skipped`. Check codes: `extraction` (item failure), `skipped` (reported as `error`), `targets` (no account to book on; the other checks are then not run), and the wizard's six checks `transaction-date`, `valid-types`, `security-values`, `duplicates`, `currencies`, `forex-gross-value`. The preview transaction is serialized with the new `EntityJson.toJsonUnbooked` (the detailed shape without `owner`/`linked`), not `toJsonDetailed`, because extracted entries have no owners until they are inserted.
  - Commit: without `select`, only `ok` items (and investment-plan items with a warning) are imported, as the wizard preselects them - not all `ok`/`warning` items as the plan's adapter row says, so that probable duplicates are never imported unasked; `select` may name `warning` items; `error` items are `422 item-not-importable` (`select[i]`, new code), bad indices `invalid-value`/`invalid-type`. Instruments are inserted first. The commit answers `200 {importId, count, items[{index, kind}], transactions[TransactionDetail of every new transaction], instruments[{uuid, name}]}`; a dry run `200 {dryRun, importId, count, items[preview item]}` and keeps the session. The wizard's `enrichMissingExDate` (a UI helper) and its follow-up price update for new instruments are not done. Logged as `MsgApiImportCommitted`.
  - Tests: the fragment has no real PDFs. `PdfImportHandler.extraction` is a package-private seam; `PdfTextExtraction` (test) runs `ComdirectPDFExtractor` on the text fixture `comdirect-Kauf02.txt` (copied from `name.abuchen.portfolio.tests/.../pdf/comdirect/Kauf02.txt`) for placeholder `.pdf` files, mirroring `PDFImportAssistant` (security cache, post-processing, missing security items). One test runs the real `PDFImportAssistant` on an unparsable placeholder to cover the `errors` path.
  - `META-INF/MANIFEST.MF` imports `org.json.simple` and `org.json.simple.parser` (used by step 17).
- Step 17 (CSV import): `POST /imports/csv` `{path, config, targets?}` answers the same preview as the PDF import (`kind: "csv"`, `errors: []`). `path` is absolute and readable (any extension). `config` is `CSVConfig` JSON; the API defaults `label` (`REST API`) and `encoding` (UTF-8 instead of the platform default) and rejects what `CSVConfig` would silently change: an unknown `target`, a `delimiter` other than `,`/`;`/tab, a column `field` the target's extractor does not know, non-numeric `skipLines` (`config.<field>` `invalid-value`/`invalid-type`). Any unreadable line fails the whole request with `422` and one `csv-parse-error` per line, field `line[N]` (N parsed from the extractor's localized "Line N: …" message; `line` if none). A price CSV (`investment-vehicle-price`) is turned into one `price` item per line (`CsvImportHandler.PriceItem`, whose instrument is set at commit); the commit requires `targets.instrument` (`required`) and adds the prices as the wizard does (`Security#addPrice`).
- Step 18 (change log): the writes of steps 3-17 already logged as they were written, so this step completes the vocabulary rather than adding call sites: `ChangeLog.detail` uses a new generic `MsgApiFieldChanged` (same text) instead of the instrument-specific key, which `InstrumentChangeLog.detail` keeps using. The action keys were added with their steps: `MsgApiStockSplitApplied`, `MsgApiPlanTransactionsGenerated`, `MsgApiPricesUpdated` (logged when the job finishes), `MsgApiImportCommitted`. `ChangeLogMessagesWellFormedTest` now checks every change-log pattern in every locale for all its placeholders, and that every locale has the English key set in sorted order; `ActionChangeLogTest` checks that the actions log one entry naming the file and that dry runs log nothing.
