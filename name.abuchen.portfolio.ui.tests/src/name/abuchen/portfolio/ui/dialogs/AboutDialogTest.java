        ...
        try (var inputStream = PortfolioPlugin.class.getResourceAsStream("dialogs/about.contributors.txt"))
        {
            ...
            try (var scanner = new Scanner(inputStream, StandardCharsets.UTF_8))
            {
                while (scanner.hasNextLine())
                {
                    ...
                    if (!line.isEmpty())
                    {
                        // Check that each line contains only one contributor (no commas, no spaces)
                        if (line.contains(","))
                            fail("Line " + lineNumber + " contains comma - each contributor must be on a separate line: " + line);

                        if (line.contains(" "))
                            fail("Line " + lineNumber + " contains spaces - each contributor must be a single username: " + line);
            ...
            if (!duplicates.isEmpty())
                fail("Found duplicate contributors: " + String.join(", ", duplicates));
        }
    }
