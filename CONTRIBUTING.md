...
- [Development Setup](#development-setup)
  ...
  - [Claude Code CLI Setup (Optional)](#claude-code-cli-setup-optional)
...
**Helpful Resources:**
- **[CLAUDE.md](CLAUDE.md)** - Project architecture, module structure, build commands
- **[CC-SESSIONS.md](CC-SESSIONS.md)** - Claude Code CLI workflow guide (optional AI assistant)
...
- [SonarLint](https://marketplace.eclipse.org/content/sonarlint) - Code quality analysis
...
[Claude Code](https://claude.ai/claude-code) is an optional AI assistant that provides task-based workflow management for Portfolio Performance development.

**What it provides:**
- **Task management** with persistent context and DAIC mode (Discussion → Implementation workflow)
- **Specialized slash commands** for common tasks:
  - `/pdf-importer BankName Kauf01.txt` - Create new PDF importer with TDD workflow
  - `/pdf-debug BankName Kauf01.txt` - Debug existing PDF importer
  - `/release-notes` - Generate bilingual release notes (German/English + XML)
- **Context compaction** - Automatically maintains task context across sessions
- **Documentation agents** - Automated code review, documentation updates

**When to use Claude Code:**
- Creating new PDF importers (faster with guided TDD workflow)
- Debugging complex PDF extraction issues
- Generating release notes
- Working on multi-session tasks with persistent context

**Setup:**

Install Claude Code CLI following the [official installation guide](https://docs.claude.com/claude-code).

**📖 Complete Guide:**

See **[CC-SESSIONS.md](CC-SESSIONS.md)** for:
- DAIC Mode workflow (Discussion → Implementation)
- Task creation and management
- Trigger phrases and branch enforcement
- Context compaction protocols
- Agent usage (context-gathering, code-review, service-documentation)
- Slash command reference

**Quick Example:**
```bash
# Create a task for new PDF importer
"Create a task for implementing Deutsche Bank PDF importer"

# Start working on the task
"Let's work on m-implement-deutsche-bank-importer"

# Switch to implementation mode
"go ahead"  # or "los geht's" in German
...
- Core module structure breakdown
- Key entry points with file paths and line numbers
- Data model concepts (entities, transactions, persistence)
...
See **[CLAUDE.md - Build Commands](CLAUDE.md#build-and-development-commands)** for more build variations and profiles.
...
4. **Understand the codebase** - Review [CLAUDE.md](CLAUDE.md) for architecture overview
...
See **[sessions/knowledge/pdf-importer.md](sessions/knowledge/pdf-importer.md)** for detailed guide:
- **5-Phase TDD Workflow** - Step-by-step development process
- **7 Mandatory Test Assertions** - Required test validations
- **Standard Forex Attributes** - Multi-currency transaction handling
- **Common Pitfalls** - Gotchas and debugging tips

**Quick Start with Claude Code:**

```bash
# Create new PDF importer task (multilingual transaction detection)
/pdf-importer BankName Kauf01.txt Dividende01.txt

# Debug existing importer
...
public void testWertpapierKauf01() {
    ...
    assertThat(results, hasItem(purchase(
        hasDate("2024-01-15"),
        hasShares(100),
        hasAmount("EUR", 1234.56),
        hasTaxes("EUR", 12.34),
        hasFees("EUR", 5.90),
        check(tx -> assertThat(tx.getSecurity().getName(), is("Example Corp.")))
    )));
}
```
...
- **[CC-SESSIONS.md](CC-SESSIONS.md)** - Claude Code workflow guide
- **[sessions/knowledge/](sessions/knowledge/)** - Specialized development guides
