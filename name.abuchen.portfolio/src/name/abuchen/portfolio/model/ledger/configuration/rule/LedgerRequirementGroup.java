...
public final class LedgerRequirementGroup
{
                    ...
                    Set<LedgerParameterType> parameterTypes, Set<LedgerPrimaryMovement> primaryMovements)
    {
        ...
        if (this.postingTypes.isEmpty() && this.parameterTypes.isEmpty() && this.primaryMovements.isEmpty())
            throw new IllegalArgumentException(LedgerDiagnosticCode.LEDGER_CORE_022
    ...
    private static String requireName(String name)
    {
        if (name == null || name.isBlank())
            throw new IllegalArgumentException(
                            LedgerDiagnosticCode.LEDGER_CORE_024.message("Ledger requirement group name is required")); //$NON-NLS-1$
