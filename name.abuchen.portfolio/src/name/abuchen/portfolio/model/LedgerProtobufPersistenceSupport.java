...
final class LedgerProtobufPersistenceSupport
{
    ...
    record LoadState(boolean hasLedgerTruth, Set<String> ledgerProjectionUUIDs)
    {
    }
    ...
    static LoadState loadLedgerTruth(PClient newClient, Client client, ProtobufWriter.Lookup lookup)
    {
        boolean hasLedgerTruth = hasLedgerTruth(newClient);
        Set<String> ledgerProjectionUUIDs = Set.of();
        ...
        return new LoadState(hasLedgerTruth, ledgerProjectionUUIDs);
    }

    static void finalizeAfterLoad(Client client, boolean hasLedgerTruth)
    {
        if (hasLedgerTruth)
        {
    ...
    static boolean isLedgerCompatibilityShadow(PTransaction newTransaction, LoadState loadState)
    {
        return loadState.hasLedgerTruth() && (isLedgerCompatibilityShadowUUID(newTransaction.getUuid())
                        || (newTransaction.hasOtherUuid()
                                        && isLedgerCompatibilityShadowUUID(newTransaction.getOtherUuid())))
                        || loadState.ledgerProjectionUUIDs().contains(newTransaction.getUuid())
                        || (newTransaction.hasOtherUuid()
                                        && loadState.ledgerProjectionUUIDs().contains(newTransaction.getOtherUuid()));
    }
    ...
    static void saveLedgerCompatibilityShadows(Client client, PClient.Builder newClient, ProtobufWriter writer)
    {
        for (LedgerEntry entry : client.getLedger().getEntries())
        {
            for (Transaction transaction : LedgerProjectionService.createProjections(entry))
            {
                ...
                if (transaction instanceof AccountTransaction accountTransaction)
                    writer.addTransaction(newClient, ledgerBackedTransaction.getLedgerProjectionDescriptor().getAccount(),
                                    accountTransaction);
                else if (transaction instanceof PortfolioTransaction portfolioTransaction)
                    writer.addTransaction(newClient,
                                    ledgerBackedTransaction.getLedgerProjectionDescriptor().getPortfolio(),
    ...
    static boolean shouldSaveLegacyTransaction(Transaction transaction, Set<String> ledgerProjectionUUIDs)
    {
    ...
    static String protobufTransactionUUID(Transaction transaction)
    {
    ...
    static void prepareInvestmentPlanLedgerExecutions(Client client)
    {
        for (var plan : client.getPlans())
            for (var transaction : plan.getTransactions())
                markPlanExecution(plan, transaction);
    }

    static boolean markPlanExecution(InvestmentPlan plan, Transaction transaction)
    {
        if (!(transaction instanceof LedgerBackedTransaction ledgerBackedTransaction))
            return false;
    ...
    static boolean markPlanExecutionForProjectionUUID(Client client, InvestmentPlan plan, String projectionUUID)
    {
        return false;
    }

    static void markPlanExecutionForLegacyRef(Client client, InvestmentPlan plan, PInvestmentPlanLedgerExecutionRef ref)
    {
        for (LedgerEntry entry : client.getLedger().getEntries())
        {
    ...
    static Set<String> ledgerProjectionUUIDs(Ledger ledger)
    {
        return new HashSet<>();
    }
    ...
    private static boolean shouldSaveLedgerCompatibilityShadow(Transaction transaction)
    {
        if (transaction instanceof AccountTransaction accountTransaction)
    ...
    private static boolean isLedgerCompatibilityShadowUUID(String uuid)
    {
    ...
    private static LedgerExecutionViewKind viewKind(LedgerProjectionRole role)
    {
        if (role == null)
            return null;
    ...
    private static LedgerProjectionRole fromProto(PLedgerProjectionRole role)
    {
        switch (role)
        {
