...
public class InsertAction implements ImportAction
{
    ...
    public Status process(BuySellEntry entry, Account account, Portfolio portfolio)
    {
        ...
        // when importing transactions that have already been generated by a savings plan
        // find the existing item and delete it - it will be created again below
        if (savingsPlanItem)
        {
            DetectDuplicatesAction action = new DetectDuplicatesAction();
            AccountTransaction existingTransaction = (AccountTransaction) action
                            .findSavingsPlanTransaction(entry.getAccountTransaction(), account.getTransactions());

            account.deleteTransaction(existingTransaction, client);
        }
