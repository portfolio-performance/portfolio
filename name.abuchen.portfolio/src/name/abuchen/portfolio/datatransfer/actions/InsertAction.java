...
public class InsertAction implements ImportAction
{
    ...
    public Status process(BuySellEntry entry, Account account, Portfolio portfolio)
    {
        ...
        if (investmentPlanItem)
        {
            ...
            // remove account transaction if import is converted to delivery
            if (convertBuySellToDelivery)
            {
                ((PortfolioTransaction) existingTransaction).setType(PortfolioTransaction.Type.DELIVERY_INBOUND);
                account.getTransactions().remove(existingTransaction);
            }
            return Status.OK_STATUS;
        }
