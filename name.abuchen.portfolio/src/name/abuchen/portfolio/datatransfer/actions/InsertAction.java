...
public class InsertAction implements ImportAction
{
    ...
    public Status process(AccountTransaction transaction, Account account)
    {
        ...
        if (removeDividends
            && transaction.getType() == AccountTransaction.Type.DIVIDENDS && transaction.getSecurity() != null)
        {
            AccountTransaction removal = new AccountTransaction(
                transaction.getDateTime().plusNanos(1), transaction.getCurrencyCode(),
                transaction.getAmount(), null, AccountTransaction.Type.REMOVAL
            );
