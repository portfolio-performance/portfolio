...
public class CSVExporter
{
    ...
    private void writeAccountTransaction(CSVPrinter printer, AccountTransaction accountTransaction) throws IOException
    {
        ...
        printer.print(Values.Amount
                        .format(accountTransaction.getType().isDebit() ? -accountTransaction.getAmount()
    ...
    private void writePortfolioTransaction(CSVPrinter printer, PortfolioTransaction transaction) throws IOException
    {
        ...
        printer.print(Values.Amount.format(transaction.getType().isLiquidation() ? -transaction.getAmount() : transaction.getAmount()));
                    ...
                    int[] prices, Optional<CurrencyConverter> converter) throws IOException
    {
        ...
        for (int i = 0; i < prices.length; i++)
        {
            ...
            {
                var security = securitiesToExport.get(i);
                long value = security.getPrices().get(prices[i]).getValue();

                if (converter.isPresent())
                    value = converter.get()
                                    .convert(dayToExport, Money.of(security.getCurrencyCode(),
                                                    value))
                                    .getAmount();
    ...
    private int[] getSecurityPricesForDay(List<Security> securitiesToExport, LocalDate dayToExport)
    {
        ...
        for (int i = 0; i < securitiesToExport.size(); i++)
        {
            var security = securitiesToExport.get(i);
            prices[i] = Collections.binarySearch(security.getPrices(), new SecurityPrice(dayToExport, 0));
        }
