...
public class ComdirectPDFExtractor extends AbstractPDFExtractor
{
    ...
    private void addBuySellTransaction()
    {
        ...
        pdfTransaction
                ...
                .match("Handelszeit ([\\s]+)?: ([\\s]+)?(?<time>\\d+:\\d+) Uhr.*")
                .assign((t, v) -> type.getCurrentContext().put("time", v.get("time")))

                // Geschäftstag      : 01.01.2000        Ausführungsplatz  : XETRA 
                .section("date")
                .match("^Gesch.ftstag ([\\s]+)?: ([\\s]+)?(?<date>\\d+.\\d+.[\\d]{4}).*$")
                .assign((t, v) -> {
                ...
                .match("^.* \\d+.\\d+.[\\d]{4} ([\\s]+)?(?<currency>[\\w]{3}) ([\\s]+)?(?<amount>[.,\\d]+)(.*)?$")
                .assign((t, v) -> {                   
                ...
                .match("^.* Zu Ihren (Lasten|Gunsten)( vor Steuern)?(.*)?$")
                .match("^.* \\d+.\\d+.[\\d]{4} ([\\s]+)?(?<fxCurrency>[\\w]{3}) ([\\s]+)?[.,\\d]+(.*)?$")
                .assign((t, v) -> {                   
                    if ("X".equals(type.getCurrentContext().get("negative")))
                    {
                        String forex = asCurrencyCode(v.get("fxCurrency"));
                ...
                .match("^.* Umrechn. zum Dev. kurs (?<exchangeRate>[.,\\d]+)(.*)?$")
                .match("^.* \\d+.\\d+.[\\d]{4} ([\\s]+)?(?<currency>[\\w]{3}) ([\\s]+)?[.,\\d]+(.*)?$")
                .assign((t, v) -> {
                    if ("X".equals(type.getCurrentContext().get("negative")))
                    {
                        // read the forex currency, exchange rate and gross
    ...
    private void addAdvanceTaxTransaction()
    {
        ...
        pdfTransaction
                ...
                .section("date")
                .match("^Die Belastung erfolgt mit Valuta (?<date>\\d+.\\d+.\\d{4}) .*$")
                .assign((t, v) -> t.setDateTime(asDate(v.get("date"))))
    ...
    private void addSellWithNegativeAmountTransaction()
    {
        ...
        pdfTransaction
                ...
                .match("^.* Kurswert ([\\s]+)?: ([\\s]+)?(?<fxCurrency>[\\w]{3}) ([\\s]+)?(?<amount>[.,\\d]+)(.*)?$")
                .match("^.* Umrechn. zum Dev. kurs (?<exchangeRate>[.,\\d]+)(.*)?$")
                .match("^.* \\d+.\\d+.[\\d]{4} ([\\s]+)?(?<currency>[\\w]{3}) ([\\s]+)?(?<amount2>[.,\\d]+)(.*)?$")
                .assign((t, v) -> {
                    if ("X".equals(type.getCurrentContext().get("negative")))
                    {
                        // read the forex currency, exchange rate and gross
    ...
    private void addDepositoryFeeTransaction()
    {
        ...
        pdfTransaction
                    ...
                    .match("^.*Verwahrentgelt (?<name>.*), WKN (?<wkn>.*) (?<date>\\d+.\\d+.\\d{4})$")
                    .assign((t, v) -> {
                        v.put("wkn", stripBlanks(v.get("wkn")));
    ...
    private void addTaxReturnBlock(DocumentType type)
    {
        ...
        block.set(new Transaction<AccountTransaction>()
                ...
                // Geschäftstag      : 01.01.2000        Ausführungsplatz  : XETRA     
                .section("date")
                .match("^Gesch.ftstag [\\s]+: (?<date>\\d+.\\d+.[\\d]{4}) .*$")
                .assign((t, v) -> {
                    if (type.getCurrentContext().get("time") != null)
                        t.setDateTime(asDate(v.get("date"), type.getCurrentContext().get("time")));
                    else
    ...
    private void addFinancialReport()
    {
        ...
        Block feeblock = new Block("^\\d+.\\d+.\\d{4} (Kontoabschluss Kontof.hrung|Geb.hren\\/Spesen|Geb.hr Barauszahlung|Entgelte|Auslandsentgelt).* [-]([.,\\d]+)$");
        ...
        Block interestblock = new Block("^\\d+.\\d+.\\d{4} (Kontoabschluss Abschluss Zinsen).* [+]([.,\\d]+)$");
        ...
        Block interestchargeblock = new Block("^\\d+.\\d+.\\d{4} (Kontoabschluss Abschluss Zinsen).* [-]([.,\\d]+)$");
        ...
        while (iterator.hasNext())
        {
            ...
            if (isDividendTransaction(i))
            {
                ...
                if (similarTransactions.size() == 2)
                {
                    ...
                    if (a2.getUnit(Type.TAX).isPresent() || !a2.getUnit(Type.TAX).isPresent())
                    {
