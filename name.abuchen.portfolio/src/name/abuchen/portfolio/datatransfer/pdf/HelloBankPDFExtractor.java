...
public class HelloBankPDFExtractor extends AbstractPDFExtractor
{
    ...
    private void addBuyTransaction()
    {
        ...
        block.set(new Transaction<BuySellEntry>()
                        ...
                        .optional() //
                        .match("Kurswert: -(?<gross>[\\d+,.-]*) (?<currency>\\w{3}+)").match("-[\\d+,.-]* \\w{3}+")
    ...
    private void addSellTransaction()
    {
        ...
        block.set(new Transaction<BuySellEntry>()
                        ...
                        .match("Kapitalertragsteuer: -(?<tax>[\\d+,.-]*) \\w{3}+").match("[\\d+,.-]* \\w{3}+") //
                        .match("Devisenkurs: (?<exchangeRate>[\\d+,.]*) \\(\\d+.\\d+.\\d{4}+\\) [\\d+,.]* \\w{3}+")
                        .assign((t, v) -> {
                            long gross = asAmount(v.get("gross"));
    ...
    private void addDividendTransaction()
    {
        ...
        block.set(new Transaction<AccountTransaction>()
                        ...
                        .section("amount", "currency").match("Zu Gunsten .* (?<amount>[\\d+,.]*) (?<currency>\\w{3}+)$")
                        .assign((t, v) -> {
                            t.setAmount(asAmount(v.get("amount")));
                            t.setCurrencyCode(asCurrencyCode(v.get("currency")));
                        })
                        ...
                        .section("gross", "currency", "tax", "exchangeRate") //
                        .optional().match("Bruttoertrag: (?<gross>[\\d+,.-]*) (?<currency>\\w{3}+)")
                        .match("KESt Ausländische Dividende: -(?<tax>[\\d+,.]*) \\w{3}+").match("[\\d+,.]* \\w{3}+")
                        .match("Devisenkurs: (?<exchangeRate>[\\d+,.]*) \\(\\d+.\\d+.\\d{4}+\\) [\\d+,.]* \\w{3}+")
                        .assign((t, v) -> {
                            ...
                            if (currency.equals(t.getSecurity().getCurrencyCode()))
                            {
                                long convertedGross = BigDecimal.valueOf(gross).multiply(exchangeRate)
                                                .setScale(0, BigDecimal.ROUND_HALF_UP).longValue();
                                t.addUnit(new Unit(Unit.Type.GROSS_VALUE, Money.of(t.getCurrencyCode(), convertedGross),
                                                Money.of(currency, gross), exchangeRate));
