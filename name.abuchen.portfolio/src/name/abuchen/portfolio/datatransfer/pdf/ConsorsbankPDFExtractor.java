...
public class ConsorsbankPDFExtractor extends AbstractPDFExtractor
{
    ...
    private void addBuySellTransaction()
    {
        ...
        pdfTransaction
                ...
                //       ST                        50,00000               WKN: 851144
                .section("shares").optional()
                .match("^[\\s]+ ST [\\s]+(?<shares>[.,\\d]+) [\\s]+WKN:.*$")
                .assign((t, v) -> t.setShares(asShares(v.get("shares"))))

                // Kauf AM 17.10.2005 IN SPARPLAN NR.2424880.001
                .section("date").optional()
                .match("^(KAUF|Kauf|BEZUG|Bezug|VERKAUF|Verkauf|VERK. TEIL-/BEZUGSR.) AM (?<date>\\d+.\\d+.\\d{4}) .*$")
                .assign((t, v) -> t.setDate(asDate(v.get("date"))))

                //       KAUF                 AM 18.09.2001 IN FRANKFURT          NR. 6201999.001
                .section("date").optional()
                .match("^[\\s]+ (KAUF|Kauf|VERKAUF|Verkauf|VERK. TEIL-/BEZUGSR.) [\\s]+AM (?<date>\\d+.\\d+.\\d{4}) .*$")
                .assign((t, v) -> t.setDate(asDate(v.get("date"))))

                // KAUF AM 15.01.2015  UM 08:13:35 MUENCHEN NR. 12345670.001
                .section("date", "time").optional()
                .match("^(KAUF|Kauf|BEZUG|Bezug|VERKAUF|Verkauf|VERK. TEIL-/BEZUGSR.) AM (?<date>\\d+.\\d+.\\d{4}) ([\\s]+)?UM (?<time>\\d+:\\d+:\\d+) .*$")
                .assign((t, v) -> {
                    if (v.get("time") != null)
                ...
                .match("^(Wert|WERT) \\d+.\\d+.\\d{4} (?<currency>[\\w]{3}) (?<amount>[.,\\d]+)$")
                .assign((t, v) -> {
                ...
                .match("^[\\s]+ WERT [\\s]+\\d+.\\d+.\\d{4} [\\s]+(?<currency>[\\w]{3}) [\\s]+(?<amount>[.,\\d]+)$")
                .assign((t, v) -> {
                    t.setCurrencyCode(asCurrencyCode(v.get("currency")));
                    t.setAmount(asAmount(v.get("amount")));
                })
    ...
    private void addDividendeTransaction()
    {
        ...
        pdfTransaction
                ...
                .match("^(Steuerfreie )?(Dividende pro St.ck|Ertragsausschüttung je Anteil) [.,\\d]+ (?<currency>[\\w]{3}) Schlusstag \\d+.\\d+.\\d{4}$")
                .assign((t, v) -> {
                    t.setShares(asShares(v.get("shares")));
                    t.setSecurity(getOrCreateSecurity(v));
                })
                ...
                .section("date", "currency", "amount").optional()
                .match("^WERT (?<date>\\d+.\\d+.\\d{4}) ([\\s]+)?(?<currency>[\\w]{3}) ([\\s]+)?(?<amount>[.,\\d]+)(.*)?$")
                .assign((t, v) -> {
                    t.setDateTime(asDate(v.get("date")));
                    t.setCurrencyCode(asCurrencyCode(v.get("currency")));
                    t.setAmount(asAmount(v.get("amount")));
    ...
    private <T extends Transaction<?>> void addFeesSectionsTransaction(T transaction, DocumentType type)
    {
        transaction
                ...
                .match("^B.rsenplatzgeb.hr (?<currency>[\\w]{3}) (?<fee>[.,\\d]+)$")
                .assign((t, v) -> processFeeEntries(t, v, type))

                // Börsenplatzgebühr 2,50 EUR
                .section("fee", "currency").optional()
                .match("^B.rsenplatzgeb.hr (?<fee>[.,\\d]+) (?<currency>[\\w]{3})$")
                .assign((t, v) -> processFeeEntries(t, v, type))
                ...
                // PROVISION EUR 8,26
                .section("fee", "currency").optional()
                .match("^(Provision|PROVISION) (?<currency>[\\w]{3}) (?<fee>[.,\\d]+)$")
                .assign((t, v) -> processFeeEntries(t, v, type))
                ...
                .section("fee", "currency").optional()
                .match("^(Grundgeb.hr|GRUNDGEBUEHR) (?<currency>[\\w]{3}) (?<fee>[.,\\d]+)$")
                .assign((t, v) -> processFeeEntries(t, v, type))
                ...
                .section("fee", "currency").optional()
                .match("^Consorsbank Ausgabegeb.hr [.,\\d]+% (?<currency>[\\w]{3}) (?<fee>[.,\\d]+)$")
                .assign((t, v) -> processFeeEntries(t, v, type))
                ...
                .section("fee", "currency").optional()
                .match("^Transaktionsentgelt (?<fee>[.,\\d]+) (?<currency>[\\w]{3})$")
                .assign((t, v) -> processFeeEntries(t, v, type))
                ...
                // Eig. Spesen EUR 1,95
                .section("fee", "currency").optional()
                .match("^Eig. Spesen (?<currency>[\\w]{3}) (?<fee>[.,\\d]+)$")
                .assign((t, v) -> processFeeEntries(t, v, type))
