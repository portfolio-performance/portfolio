...
public class OberbankAGPDFExtractor extends AbstractPDFExtractor
{
    ...
    private void addBuySellTransaction()
    {
        ...
        pdfTransaction //
                        ...
                        // Is type --> "Verkauf" or "Rücknahme Fonds" change from BUY to SELL
                        ...
                        .oneOf( //
                                        ...
                                        // SVK Sammelverwahrung Wert 11.03.2021 EUR              239,93
                                        // @formatter:on
                                        section -> section //
                        ...
                        .match("^Kupon [\\.,\\d]+ % .* St.ckzinsen (?<note2>f\\. [\\d]+ Tage).* (?<note4>[A-Z]{3})[\\s]+(?<note3>[\\.,\\d]+)$") //
                        .assign((t, v) -> {
                            t.setNote(concatenate(t.getNote(), "Stückzinsen " + v.get("note2"), " | "));
    ...
    private void addDividendTransaction()
    {
        ...
        pdfTransaction //
                        ...
                        // If we have a positive amount and a gross reinvestment, there is a
                        // tax refund. If the amount is negative, then it is taxes.
                        ...
                        .assign((t, v) -> {
                            if ("Gunsten".equals(v.get("sign")))
                                t.setType(AccountTransaction.Type.TAX_REFUND);

                            if ("Lasten".equals(v.get("sign")))
                        ...
                        .oneOf( //
                                        ...
                                        section -> section //
                                                        ...
                                                        .match("^(?<nameContinued>.*)$") //
                                                        .match("^Ertrag [\\.,\\d]+ (?<currency>[A-Z]{3})[\\s]*$") //
                                                        .assign((t, v) -> t.setSecurity(getOrCreateSecurity(v))),
                                        // @formatter:off
                                        ...
                                        section -> section //
                                                        ...
                                                        .match("^(?<nameContinued>.*)$") //
                                                        .match("^Ertrag f. [\\d]+ Tage (?<currency>[A-Z]{3})[\\s]+[\\.,\\d]+[\\s]*$") //
                                                        .assign((t, v) -> t.setSecurity(getOrCreateSecurity(v))))
                        ...
                        .optionalOneOf( //
                                        ...
                                        section -> section //
                                                        ...
                                                        .match("^Ertrag (?<termCurrency>[A-Z]{3})[\\s]+(?<fxGross>[\\.,\\d]+)$") //
                                                        .match("^.* a (?<exchangeRate>[\\.,\\d]+) v\\. [\\d]{2}\\.[\\d]{2}\\.[\\d]{4} (?<baseCurrency>[A-Z]{3})[\\s]+[\\.,\\d]+$") //
                                                        .assign((t, v) -> {
                                        ...
                                        // @formatter:off
                                        // Zwischensumme USD               22,78
                                        // 0,0006 EUR KESt a 0,9976 v. 07.11.2022 EUR               22,83
                                        // @formatter:on
                                        section -> section //
                                                        .attributes("termCurrency", "fxGross", "exchangeRate", "baseCurrency") //
                                                        .match("^Zwischensumme (?<termCurrency>[A-Z]{3})[\\s]+(?<fxGross>[\\.,\\d]+)$") //
                                                        .match("^.* a (?<exchangeRate>[\\.,\\d]+) v\\. [\\d]{2}\\.[\\d]{2}\\.[\\d]{4} (?<baseCurrency>[A-Z]{3})[\\s]+[\\.,\\d]+$") //
                                                        .assign((t, v) -> {
                                                            var rate = asExchangeRate(v);
                                                            type.getCurrentContext().putType(rate);
    ...
    private void addDeliveryInOutBoundTransaction()
    {
        ...
        // The document contains a single transaction, but repeats the header
        // line on every page. The block therefore spans the whole document.
        ...
        pdfTransaction //
                        ...
                        .section("type").optional() //
                        .match("^Durchf.hrungsanzeig[\\s]*e[\\s]+(?<type>Freie Lieferung)$") //
                        .assign((t, v) -> t.setType(PortfolioTransaction.Type.DELIVERY_OUTBOUND))
                        ...
                        .match("^(?<isin>[A-Z]{2}[A-Z0-9]{9}[0-9]) (?<name>.*) (Zugang|Abgang) Stk[\\s]*\\.[\\s]+[\\.,\\d]+$") //
                        .match("^(?<nameContinued>.*)$") //
                        .match("^[\\s]*Anschaffungskurswert:[\\s]+[\\.,\\d]+ (?<currency>[A-Z]{3})$") //
                        .assign((t, v) -> t.setSecurity(getOrCreateSecurity(v)))
    ...
    {
        // The document is headed "Kapitalmaßnahme", but the booking that has to
        // be reported is the "Einbuchung" on the following page. The kind of
        // corporate action is only named on the first page, therefore it is
        // read into the document context.
        final var type = new DocumentType("(Durchf.hrungsanzeig[\\s]*e[\\s]+" //
                        + "Kapitalma.nahme)", //
                        documentContext -> documentContext //
                                        .section("transaction").optional() //
        ...
        var firstRelevantLine = new Block("^Durchf.hrungsanzeig[\\s]*e[\\s]+Einbuchung$");
        type.addBlock(firstRelevantLine);
        firstRelevantLine.set(pdfTransaction);
        ...
        pdfTransaction //
                        ...
                        // 888888 - 20.07.2021
                        //   steuerlicher Anschaffungswert:                                                 1.298,55 EUR
                        // @formatter:on
                        .section("isin", "name", "nameContinued", "date", "shares", "currency") //
                        .documentContextOptionally("transaction") //
                        .match("^(?<isin>[A-Z]{2}[A-Z0-9]{9}[0-9]) (?<name>.*) Zugang Stk[\\s]*\\.[\\s]+(?<shares>[\\.,\\d]+)$") //
                        .match("^(?<nameContinued>.*)$") //
                        .match("^[\\d]+ - (?<date>[\\d]{2}\\.[\\d]{2}\\.[\\d]{4})$") //
                        .match("^[\\s]*steuerlicher Anschaffungswert:[\\s]+[\\.,\\d]+ (?<currency>[A-Z]{3})$") //
                        .assign((t, v) -> {
    ...
    private <T extends Transaction<?>> void addTaxesSectionsTransaction(T transaction, DocumentType type)
    {
        ...
        transaction //
                        ...
                        // @formatter:on
                        .section("tax", "currency").optional() //
                        .match("^Auslands-KESt (?<currency>[A-Z]{3})[\\s]+(\\-)?(?<tax>[\\.,\\d]+)$") //
                        .assign((t, v) -> {
                            if (!type.getCurrentContext().getBoolean("noTax"))
                                processTaxEntries(t, v, type);
                        })
                        ...
                        // @formatter:on
                        .section("tax", "currency").optional() //
                        .match("^Ausl.ndische Steuern (?<currency>[A-Z]{3})[\\s]+(\\-)?(?<tax>[\\.,\\d]+)$") //
                        .assign((t, v) -> {
                            if (!type.getCurrentContext().getBoolean("noTax"))
                                processTaxEntries(t, v, type);
                        })
                        ...
                        // @formatter:on
                        .section("tax", "currency").optional() //
                        .match("^Kupon j.hrlich KESt (?<currency>[A-Z]{3})[\\s]+(\\-)?(?<tax>[\\.,\\d]+)$") //
                        .assign((t, v) -> {
                            if (!type.getCurrentContext().getBoolean("noTax"))
                                processTaxEntries(t, v, type);
                        });
