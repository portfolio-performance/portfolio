    ...
    private void addDividendeTransaction()
    {
        ...
        pdfTransaction
                ...
                .oneOf(
                                ...
                                ,
                                // Ex-Tag : 26. Januar 2010 Dividende
                                // Netto : EUR 24,97
                                section -> section
                                        .attributes("currency", "amount")
                ...
                // Gesamtbetrag : USD 0.45
                // Devisenkurs : 0.888889
                // Gesamtbetrag (in : EUR 0.4
                .section("exchangeRate", "fxAmount", "fxCurrency", "amount", "currency").optional()
                .match("^Gesamtbetrag : (?<fxCurrency>[\\w]{3}) (?<fxAmount>[\\.',\\d\\s]+)$")
                .match("^Devisenkurs : (?<exchangeRate>[\\.',\\d\\s]+)$")
                .match("^Gesamtbetrag \\(in : (?<currency>[\\w]{3}) (?<amount>[\\.',\\d\\s]+)$")
                .assign((t, v) -> {
                    BigDecimal exchangeRate = asExchangeRate(v.get("exchangeRate"));
                ...
                .match("^(.* )?Dividende Netto : (?<fxCurrency>[\\w]{3}) (?<fxAmount>[\\.',\\d\\s]+)$")
                .match("^(.* )?Devisenkurs : (?<exchangeRate>[\\.',\\d\\s]+)$")
                .match("^(.* )?Dividende Netto : (?<currency>[\\w]{3}) (?<amount>[\\.',\\d\\s]+)$")
                .assign((t, v) -> {
                    BigDecimal exchangeRate = asExchangeRate(v.get("exchangeRate"));
