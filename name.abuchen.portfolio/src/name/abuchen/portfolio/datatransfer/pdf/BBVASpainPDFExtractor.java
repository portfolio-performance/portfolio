...
public class BBVASpainPDFExtractor extends AbstractPDFExtractor
{
    ...
    private void addBuySellTransaction()
    {
        ...
        var firstRelevantLine = new Block(
                        "^Te facilitamos el detalle de la operación (VENTA|COMPRA) DE VALORES  que hemos liquidado en tu cuenta.$");
        ...
        pdfTransaction.subject(() -> {
            ...
            return portfolioTransaction;
                        ...
                        .match("^Te facilitamos el detalle de la operación (?<type>(VENTA|COMPRA)) DE VALORES  que hemos liquidado en tu cuenta.$") //
                        .assign((t, v) -> {
            ...
            // ORIGEN USD            2.100,00
            // @formatter:on
                        .section("isin", "name", "currency").match("^Valor (?<name>.*)$") //
            ...
            // @formatter:on
                        .section("date", "time") //
                        .match("^Fecha ejecución (?<date>[\\d]{1,2}/[\\d]{1,2}/[\\d]{4})$") //
                        .match("^Hora ejecución (?<time>[\\d]{2}.[\\d]{2}.[\\d]{2})$") //
                        .assign((t, v) -> t.setDate(asDate(v.get("date"), v.get("time").replace('.', ':'))))
            ...
            // @formatter:on
                        ...
                        .match("^IMPORTE TOTAL (?<currency>[A-Z]{3})\\s+(\\-)?(?<amount>[\\d.,]+)$")
                        .assign((t, v) -> {
            ...
            // @formatter:on
                        ...
                        .match("^S/títulos [A-Z]{3}\\s+[\\d]+\\s+[\\d.,]+\\s+(\\-)?(?<fxGross>[\\.,\\d]+)$") //
                        .assign((t, v) -> {
    ...
    private void addBuySellFundTransaction()
    {
        var type = new DocumentType("OPERACIONES DE FONDOS (SUSCRIPCIÓN|REEMBOLSO) EN EFECTIVO");
        this.addDocumentTyp(type);

        var pdfTransaction = new Transaction<BuySellEntry>();

        var firstRelevantLine = new Block(
                        "^CARTA DE AVISO POR OPERACIONES DE FONDOS (SUSCRIPCIÓN|REEMBOLSO) EN EFECTIVO$");
        type.addBlock(firstRelevantLine);
        firstRelevantLine.set(pdfTransaction);
        ...
        pdfTransaction.subject(() -> {
            ...
            return portfolioTransaction;
                        ...
                        .section("type").optional() //
                        .match("^CARTA DE AVISO POR OPERACIONES DE FONDOS (?<type>(SUSCRIPCIÓN|REEMBOLSO)) EN EFECTIVO$") //
                        .assign((t, v) -> {
            ...
            // @formatter:on
                        .section("shares") //
                        .match("^[\\d]+ [\\d]+ [\\d]+ [\\d]+ [\\p{L}0-9 .]+ (?<shares>[\\d.,]+) [A-Z]{3}/[A-Z]{3}$") //
                        .assign((t, v) -> t.setShares(asShares(v.get("shares"))))
            ...
            // @formatter:on
                        ...
                        .assign((t, v) -> t.setDate(asDate(v.get("date"), "00:00:00")))
    ...
    private void addDividendeTransaction()
    {
        ...
        // @formatter:off
            ...
            // @formatter:on
                        ...
                        .match("^\\d+ \\d+ \\d+ \\d+ (?<name>[\\p{L}0-9 .]+) \\d{2}/\\d{2}/\\d{4} [\\d,]+\\s+[A-Z]{3}/[A-Z]{3}$") //
            ...
            // @formatter:on
                        .section("date") //
                        .match("^\\d+ \\d+ \\d+ \\d+ [\\p{L}0-9 .]+ (?<date>[\\d]{2}/[\\d]{2}/[\\d]{4}) [\\d,]+ [A-Z]{3}/[A-Z]{3}$") //
            ...
            // @formatter:on
                        .section("fxGross", "termCurrency", "baseCurrency", "exchangeRate") //
                        .match("^\\d+ \\d+ \\d+ \\d+ [A-Z0-9 .]+ \\d{2}/\\d{2}/\\d{4} (?<exchangeRate>[\\.,\\d]+) (?<termCurrency>[A-Z]{3})\\/(?<baseCurrency>[A-Z]{3})$") //
    ...
    private <T extends Transaction<?>> void addFeesSectionsTransaction(T transaction, DocumentType type)
    {
        ...
        // @formatter:off
            // GESTION DIVISA(SOBRE
            // SUBTOTAL EN ORIGEN) EUR                8,13
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^SUBTOTAL EN ORIGEN\\) (?<currency>[A-Z]{3})\\s+(?<fee>[\\.,\\d]+)$") //
            ...
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^BOLSA \\(comisión fija\\) (?<currency>[A-Z]{3})\\s+[\\d.,]+\\s+(?<fee>[\\.,\\d]+)$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
            ...
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^CANALIZADOR S/importe (?<currency>[A-Z]{3})\\s+[\\d.,]+\\s+[\\d.,]+%\\s+-(?<fee>[\\.,\\d]+)$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
            ...
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^Mínimo (?<currency>[A-Z]{3})\\s+-(?<fee>[\\.,\\d]+)$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
            ...
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^COMISION BANCARIA Mínimo (?<currency>[A-Z]{3})\\s+-(?<fee>[\\.,\\d]+)$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
            ...
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^I. V. A. % S/comisión bancaria (?<currency>[A-Z]{3}) [\\.,\\d]+ [\\.,\\d]+% -(?<fee>[\\.,\\d]+)$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
            ...
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^COMUNICACION (?<currency>[A-Z]{3})\\s+-(?<fee>[\\.,\\d]+)$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))

                        // @formatter:off
            // GESTION DIVISA(SOBRE
            // SUBTOTAL EN ORIGEN) EUR               -4,88
            // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^SUBTOTAL EN ORIGEN\\) (?<currency>[A-Z]{3})\\s+-(?<fee>[\\.,\\d]+)$") //
                        .assign((t, v) -> processFeeEntries(t, v, type));
