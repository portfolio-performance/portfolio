...
public class ConsorsbankPDFExctractor extends AbstractPDFExtractor
{
    ...
    private Transaction<AccountTransaction> newDividendTransaction(DocumentType type)
    {
        return new Transaction<AccountTransaction>()
                        ...
                        .assign((t, v) -> {
                            ...
                            BigDecimal rate = v.asExchangeRate(v.get("rate"));
                            v.getCurrentContext().put("exchangeRate", rate.toPlainString());
                            BigDecimal accountMoneyValue = BigDecimal.valueOf(t.getAmount()).divide(rate,
                                            RoundingMode.HALF_DOWN);
                            String currencyCode = v.asCurrencyCode(v.get("currency"));
                            t.setMonetaryAmount(Money.of(currencyCode, v.asAmount(v.get("amount"))));
                        ...
                        .assign((t, v) -> t.addUnit(new Unit(Unit.Type.TAX,
                                        Money.of(v.asCurrencyCode(v.get("currency")), v.asAmount(v.get("kapst"))))))
                        ...
                        .assign((t, v) -> {
                            ...
                            if (currency.equals(t.getCurrencyCode()))
                            {
                                t.addUnit(new Unit(Unit.Type.TAX,
                                                Money.of(v.asCurrencyCode(currency), v.asAmount(v.get("solz")))));
                            }
                        })
                        ...
                        .assign((t, v) -> {
                            ...
                            Money money = Money.of(v.asCurrencyCode(v.get("currency")), v.asAmount(v.get("qust")));
                            if (grossValueOption.isPresent())
                            {
                                Money forex = Money.of(v.asCurrencyCode(v.get("forexcurrency")),
                                                v.asAmount(v.get("forex")));
                        ...
                        .assign((t, v) -> {
                            String currencyCode = v.asCurrencyCode(v.get("currency"));
                            Money money = Money.of(currencyCode, v.asAmount(v.get("amount")));
                        ...
                        .assign((t, v) -> {
                            Optional<Unit> grossValueOption = t.getUnit(Type.GROSS_VALUE);
                            long forexAmount = v.asAmount(v.get("forexpenses"));
                            if (grossValueOption.isPresent())
                            {
                                ...
                                Money forex = Money.of(v.asCurrencyCode(v.get("currency")), forexAmount);
                                t.addUnit(new Unit(Unit.Type.TAX, money, forex,
                                                grossValueOption.get().getExchangeRate()));
                            }
                            else
                            {
                                BigDecimal exchangeRate = new BigDecimal(v.getCurrentContext().get("exchangeRate"));
    ...
    private <T extends Transaction<?>> void addTaxesSectionsTransaction(T pdfTransaction)
    {
        pdfTransaction.section("tax", "currency").optional()
                        ...
                        .assign((t, v) -> {
                            if (t instanceof name.abuchen.portfolio.model.Transaction)
                            {
                                ((name.abuchen.portfolio.model.Transaction) t).addUnit(new Unit(Unit.Type.TAX,
                                                Money.of(v.asCurrencyCode(v.get("currency")), v.asAmount(v.get("tax")))));
                            }
                            else
                            {
                                ((name.abuchen.portfolio.model.BuySellEntry) t).getPortfolioTransaction().addUnit(
                                                new Unit(Unit.Type.TAX, Money.of(v.asCurrencyCode(v.get("currency")),
                                                                v.asAmount(v.get("tax")))));
                            }
                        })
                        ...
                        .assign((t, v) -> {
                            if (t instanceof name.abuchen.portfolio.model.Transaction)
                            {
                                ((name.abuchen.portfolio.model.Transaction) t).addUnit(new Unit(Unit.Type.TAX, Money
                                                .of(v.asCurrencyCode(v.get("currency")), v.asAmount(v.get("solz")))));
                            }
                            else
                            {
                                ((name.abuchen.portfolio.model.BuySellEntry) t).getPortfolioTransaction().addUnit(
                                                new Unit(Unit.Type.TAX, Money.of(v.asCurrencyCode(v.get("currency")),
                                                                v.asAmount(v.get("solz")))));
                            }
                        })
                        ...
                        .assign((t, v) -> {
                            if (t instanceof name.abuchen.portfolio.model.Transaction)
                            {
                                ((name.abuchen.portfolio.model.Transaction) t).addUnit(new Unit(Unit.Type.TAX, Money.of(
                                                v.asCurrencyCode(v.get("currency")), v.asAmount(v.get("kirchenst")))));
                            }
                            else
                            {
                                ((name.abuchen.portfolio.model.BuySellEntry) t).getPortfolioTransaction().addUnit(
                                                new Unit(Unit.Type.TAX, Money.of(v.asCurrencyCode(v.get("currency")),
                                                                v.asAmount(v.get("kirchenst")))));
                            }
                        });
    }
    ...
    private void addFeesSectionsTransaction(Transaction<BuySellEntry> pdfTransaction)
    {
        pdfTransaction.section("currency", "stockfees").optional()
                        ...
                        .assign((t, v) -> t.getPortfolioTransaction().addUnit(new Unit(Unit.Type.FEE,
                                        Money.of(v.asCurrencyCode(v.get("currency")), v.asAmount(v.get("expenses"))))));
    }
    ...
    private void addTaxAdjustmentTransaction()
    {
        ...
        block.set(new Transaction<AccountTransaction>().subject(() -> {
            ...
            return t;
                        ...
                        .find(" *=* *")
                        .match(" *(?<amount>[\\d.]+,\\d{2}) *")
                        .assign((t, v) -> t.setAmount(v.asAmount(v.get("amount"))))
