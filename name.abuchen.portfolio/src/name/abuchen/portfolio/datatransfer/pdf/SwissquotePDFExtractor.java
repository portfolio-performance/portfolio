...
public class SwissquotePDFExtractor extends SwissBasedPDFExtractor {
        ...
        block.set(new Transaction<BuySellEntry>()
                ...
                .assign((t, v) -> {
                    ...
                    if (forex.getCurrencyCode().equals(t.getPortfolioTransaction().getSecurity().getCurrencyCode())) {
                        Unit unit;
                        // Swissquote sometimes uses scaled exchanges rates (such as DKK/CHF 15.42, instead of 0.1542,
                        // hence we try to extract and if we fail, we calculate the exchange rate
                        try {
    ...
    private void addDividendsTransaction() {
        ...
        Transaction<AccountTransaction> trans = new Transaction<AccountTransaction>()
                .subject(() -> {
