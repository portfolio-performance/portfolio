...
public final class PDFLayoutMatrixProjectionBuilder
{
    private static final int COLUMN_TOLERANCE = 50;

    public PDFLayoutMatrixProjection build(PDFLayoutTableRegion table)
    {
        List<PDFLayoutMatrixRow> rows = new ArrayList<>();

        rows.add(new PDFLayoutMatrixRow(0, buildCells(table, table.header(), -1)));

        int rowIndex = 1;

        for (PDFLayoutSegmentRow row : table.rows())
            rows.add(new PDFLayoutMatrixRow(rowIndex++, buildCells(table, row, rowIndex)));

        return new PDFLayoutMatrixProjection(table, List.copyOf(rows));
    }
