...
public class QuirinBankAGPDFExtractor extends AbstractPDFExtractor
{
    ...
    {
        var type = new DocumentType("Abrechnungskonditionen");
        this.addDocumentTyp(type);

        var pdfTransaction = new Transaction<BuySellEntry>();

        var firstRelevantLine = new Block("^(Kauf|Verkauf)$");
        type.addBlock(firstRelevantLine);
        firstRelevantLine.set(pdfTransaction);
        ...
        pdfTransaction //
                        ...
                        .match("^.* Steuerbetrag .*\\-([\\s])?(?<tax>[\\.,\\d]+) (?<currency>[A-Z]{3}).*$") //
                        .assign((t, v) -> {
                        ...
                        .match("^.* Kurswert[\\s]{1,}(\\-)?(?<fxGross>[\\.,\\d]+) (?<termCurrency>[A-Z]{3})[\\s]{1,}(\\-)?(?<gross>[\\.,\\d]+) (?<baseCurrency>[A-Z]{3}).*$") //
    ...
    {
        var type = new DocumentType("F.r aus Ihrem Depot f.llig gewordene Ertr.gnisse", "Vorabpauschale");
        this.addDocumentTyp(type);

        var pdfTransaction = new Transaction<AccountTransaction>();

        var firstRelevantLine = new Block("^Ertr.gnisabrechnung$", "^Der Abrechnungsbetrag wird mit Valuta.*$");
        type.addBlock(firstRelevantLine);
        firstRelevantLine.set(pdfTransaction);
        ...
        pdfTransaction //
                        ...
                        .oneOf( //
                                        ...
                                        section -> section //
                                                        ...
                                                        .match("^Ausmachender Betrag [A-Z]{3} [\\.,\\d]+$") //
                                                        .match("^Ausmachender Betrag (?<currency>[A-Z]{3}) (?<amount>[\\.,\\d]+)$") //
                                                        .assign((t, v) -> {
                                                            t.setCurrencyCode(asCurrencyCode(v.get("currency")));
                                                            t.setAmount(asAmount(v.get("amount")));
    ...
    {
        var type = new DocumentType("(Dividendenabrechnung" //
                        + "|Ertrag aus Investments)");
        this.addDocumentTyp(type);

        var pdfTransaction = new Transaction<AccountTransaction>();

        var firstRelevantLine = new Block("^(Dividendenabrechnung" //
                        + "|Ertrag aus Investments)$");
        type.addBlock(firstRelevantLine);
        firstRelevantLine.set(pdfTransaction);
        ...
        pdfTransaction //
                        ...
                        .oneOf( //
                                        ...
                                        section -> section //
                                                        ...
                                                        .match("^Nominal\\/St.ck (?<name>.*)$") //
                                                        .match("^ST [\\.,\\d]+ ISIN (?<isin>[A-Z]{2}[A-Z0-9]{9}[0-9])$") //
                                                        .match("^(Dividenden|Aussch.ttung) pro St.ck[\\s]{1,}[\\.,\\d]+ (?<currency>[A-Z]{3})$") //
                                                        .assign((t, v) -> t.setSecurity(getOrCreateSecurity(v))),
                                        // @formatter:off
                        ...
                        // @formatter:on
                        .section("amount", "currency") //
                        .match("^.* Bruttobetrag .* (?<amount>[\\.,\\d]+) (?<currency>[A-Z]{3})$") //
                        .assign((t, v) -> {
                            t.setCurrencyCode(asCurrencyCode(v.get("currency")));
                            t.setAmount(asAmount(v.get("amount")));
                        })
                        ...
                        // @formatter:on
                        .section("tax", "currency").optional() //
                        .match("^Ausl\\. Quellensteuer .*\\-([\\s])?(?<tax>[\\.,\\d]+) (?<currency>[A-Z]{3}).*$") //
                        .assign((t, v) -> {
                            var tax = Money.of(asCurrencyCode(v.get("currency")), asAmount(v.get("tax")));
                        ...
                        // @formatter:on
                        .section("fxGross", "termCurrency", "gross", "baseCurrency", "exchangeRate").optional() //
                        .match("^.* Bruttobetrag[\\s]{1,}(?<fxGross>[\\.,\\d]+) (?<termCurrency>[A-Z]{3})[\\s]{1,}(?<gross>[\\.,\\d]+) (?<baseCurrency>[A-Z]{3}).*$") //
                        .match("^Devisenkurs[\\s]{1,}(?<exchangeRate>[\\.,\\d]+).*$") //
                        .assign((t, v) -> {
                            var rate = asExchangeRate(v);
                            type.getCurrentContext().putType(rate);
        ...
        pdfTransaction //
                        ...
                        .oneOf( //
                                        ...
                                        section -> section //
                                                        ...
                                                        .match("^ISIN (?<isin>[A-Z]{2}[A-Z0-9]{9}[0-9])$") //
                                                        .match("^WKN (?<wkn>[A-Z0-9]{6})$") //
                                                        .match("^Vorabpauschale (?<currency>[A-Z]{3}) [\\.,\\d]+.*$") //
                                                        .assign((t, v) -> t.setSecurity(getOrCreateSecurity(v))),
                                        // @formatter:off
                                        //
                                        // @formatter:on
    ...
    {
        var type = new DocumentType("Kontoauszug");
        this.addDocumentTyp(type);
        ...
        // @formatter:on
        var depositBlock = new Block("^(Konto.bertrag [\\d]+" //
                        + "|Sammelgutschrift" //
                        + "|Interne Buchung" //
        ...
        var removalBlock = new Block("^(R.ck.berweisung Inland" //
                        + "|.berweisungsauftrag,) " //
                        + ".* \\-[\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // Steuern auf Kontoabschluss
        // @formatter:on
        var taxesBlock = new Block("^Steuerbuchung Abgeltungsteuer, .* \\-[\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // Ref.: 464710285
        // @formatter:on
        var taxRefundBlock01 = new Block("^Steueroptimierung .* [\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // Steuern auf Kontoabschluss
        // @formatter:on
        var taxRefundBlock02 = new Block("^Steuerbuchung Abgeltungsteuer, .* [\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // @formatter:on
        var feesBlock01 = new Block("^Verm.gensverwaltungshonorar.* \\-[\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // @formatter:on
        var feesBlock02 = new Block("^Flatrate, .* \\-[\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // @formatter:on
        var feesBlock03 = new Block("^Volumen Fee, .* \\-[\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // @formatter:on
        var feesBlock05 = new Block("^Honorar, .* \\-[\\.,\\d]+[\\s]{1,}[A-Z]{3}.*$");
        ...
        // @formatter:on
        var feeRefundBlock = new Block("^R.ckverg.tung Bestand.*, .* [\\.,\\d]+([\s]+)?[A-Z]{3}.*$");
        ...
        // @formatter:on
        var interestBlock = new Block("^Haben\\-Zinsen Kontoabschluss, .* [\\.,\\d]+([\\s]+)?[A-Z]{3}.*$");
    ...
    private void addTaxLostAdjustmentTransaction(Map<String, String> context, DocumentType type)
    {
        ...
        pdfTransaction //
                        ...
                        // @formatter:on
                        .section("note").optional() //
                        .match("^.* (?<note>Zwischengewinn[\\s]{1,}[\\.,\\d]+ [A-Z]{3}).*$") //
    ...
    private <T extends Transaction<?>> void addTaxesSectionsTransaction(T transaction, DocumentType type)
    {
        transaction //
                        ...
                        .match("^Ausl\\. Quellensteuer .*\\-([\\s])?(?<withHoldingTax>[\\.,\\d]+) (?<currency>[A-Z]{3}).*$") //
                        ...
                        .match("^Kapitalertrags(s)?teuer (?<currency>[A-Z]{3}) \\-([\\s])?(?<tax>[\\.,\\d]+).*$") //
                        ...
                        // @formatter:on
                        .section("currency", "tax").optional() //
                        .match("^Solidarit.tszuschlag (?<currency>[A-Z]{3}) \\-([\\s])?(?<tax>[\\.,\\d]+).*$") //
                        .assign((t, v) -> processTaxEntries(t, v, type))
                        ...
                        // @formatter:on
                        .section("currency", "tax").optional() //
                        .match("^Kirchensteuer (?<currency>[A-Z]{3}) \\-([\\s])?(?<tax>[\\.,\\d]+).*$") //
                        .assign((t, v) -> processTaxEntries(t, v, type))
    ...
    private <T extends Transaction<?>> void addFeesSectionsTransaction(T transaction, DocumentType type)
    {
        transaction //
                        ...
                        .match("^Abwicklungsgeb.hren \\* (?<currency>[A-Z]{3}) \\-([\\s])?(?<fee>[\\.,\\d]+).*$") //
                        ...
                        .match("^.*Abwickl\\.Geb.hr \\* .*\\-([\\s])?(?<fee>[\\.,\\d]+) (?<currency>[A-Z]{3}).*$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
                        ...
                        // @formatter:on
                        .section("currency", "fee").optional() //
                        .match("^Courtage \\* (?<currency>[A-Z]{3}) \\-([\\s])?(?<fee>[\\.,\\d]+).*$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
                        ...
                        // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^.* .* Courtage \\* .*\\-([\\s])?(?<fee>[\\.,\\d]+) (?<currency>[A-Z]{3}).*$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
                        ...
                        // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^.*Spesen \\* \\-([\\s])?(?<fee>[\\.,\\d]+) (?<currency>[A-Z]{3}).*$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
                        ...
                        // @formatter:on
                        .section("currency", "fee").optional() //
                        .match("^.*Bank\\-Provision (?<currency>[A-Z]{3}) \\-([\\s])?(?<fee>[\\.,\\d]+).*$") //
                        .assign((t, v) -> processFeeEntries(t, v, type))
                        ...
                        // @formatter:on
                        .section("fee", "currency").optional() //
                        .match("^.* Bank\\-Provision .*\\-([\\s])?(?<fee>[\\.,\\d]+) (?<currency>[A-Z]{3}).*$") //
                        .assign((t, v) -> processFeeEntries(t, v, type));
    }
}
