...
public class OpenBankSAPDFExtractor extends AbstractPDFExtractor
{
    ...
    private void addBuySellTransaction()
    {
        ...
        pdfTransaction
                ...
                .match("^.*A([\\s]+)?N([\\s]+)?G([\\s]+)?E([\\s]+)?G([\\s]+)?E([\\s]+)?B([\\s]+)?E([\\s]+)?N ([\\s]+)?I([\\s]+)?N ([\\s]+)?: ([\\s]+)?"
                                + "(?<currency>[\\w]{3})$")
                .match("^.*D([\\s]+)?A([\\s]+)?T([\\s]+)?U([\\s]+)?M([\\s]+)?: ([\\s]+)?"
                                + "[\\d]{2}\\-[\\d]{2}-[\\d]{4} "
                                + "(?<name>.*) ([\\s]+)?"
                                + "W([\\s]+)?E([\\s]+)?C([\\s]+)?H([\\s]+)?S([\\s]+)?E([\\s]+)?L([\\s]+)?K([\\s]+)?U([\\s]+)?R([\\s]+)?S([\\s]+)?: ([\\s]+)?"
                                + "[\\.,\\d]+$")
                .match("^.*I([\\s]+)?S([\\s]+)?I([\\s]+)?N([\\s]+)?: ([\\s]+)?"
                ...
                // @formatter:on
                .section("shares")
                .match("^.*BETEILIGUNGEN ([\\s]+)?(?<shares>[\\.,\\d]+)$")
                ...
                // @formatter:on
                .section("date")
                .match("^.*DATUM NETTOINVENTARWERT ([\\s]+)?(?<date>[\\d]{2}\\.[\\d]{2}\\.[\\d]{4})$")
                ...
                .match("^.*NETTOBETRAG ([\\s]+)?(?<amount>[\\.,\\d]+) (?<currency>[\\w]{3})$")
                .assign((t, v) -> {
                ...
                .match("^.*: (?<note>[\\d\\s]+) ([\\s]+)?PERS.NLICHES KONTO$")
