...
public class MeineBankPDFExtractor extends AbstractPDFExtractor
{

    private static final String DEPOSIT_2023 = "^(?<date>[\\d]{2}\\.[\\d]{2}\\.) ([\\d]{2}\\.[\\d]{2}\\.) (.*UEBERWEISUNG) (PN:\\d+) (?<amount>[\\.\\d]+,[\\d]{2}) [H]";
    private static final String REMOVAL_2023 = "^(?<date>[\\d]{2}\\.[\\d]{2}\\.) ([\\d]{2}\\.[\\d]{2}\\.) (.*UEBERWEISUNG) (PN:\\d+) (?<amount>[\\.\\d]+,[\\d]{2}) [S]";
    private static final String INTEREST_2023 = "^(?<date>[\\d]{2}\\.[\\d]{2}\\.) ([\\d]{2}\\.[\\d]{2}\\.) (Abschluss lt\\. Anlage \\d) (PN:\\d+) (?<amount>[\\.\\d]+,[\\d]{2}) (.*)";
    private static final String TAXES_2023 = "^(?<date>[\\d]{2}\\.[\\d]{2}\\.) ([\\d]{2}\\.[\\d]{2}\\.) (Kapitalertragsteuer|Solid\\.-Zuschlag|Kirchensteuer) (\\w+) (PN:\\d+) (?<amount>[\\d\\s,.]*?) [S]";
    ...
    public MeineBankPDFExtractor(Client client)
    {
        super(client);

        addBankIdentifier("Raiffeisenbank im Hochtaunus eG");
    ...
    {
        return "Raiffeisenbank im Hochtaunus eG";
    }

    private void addTransactionWith2023Format()
    {
        DocumentType type = new DocumentType("Raiffeisenbank im Hochtaunus eG", contextProvider2023());
        ...
        Block taxesBlock = new Block(TAXES_2023);
        taxesBlock.set(taxesTransaction(type, TAXES_2023));
    ...
    private BiConsumer<AccountTransaction, ParsedData> assignmentsProvider(DocumentType type)
    {
        return (transaction, matcherMap) -> {
            ...
            transaction.setCurrencyCode(context.get(MeineBankPDFExtractor.CONTEXT_KEY_CURRENCY));
        };
    }
