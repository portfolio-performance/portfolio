...
public final class BVBQuoteFeed implements QuoteFeed
{
    public static final String ID = "BVB"; //$NON-NLS-1$
    ...
    {
        return "Bursa de Valori Bucuresti"; //$NON-NLS-1$
    }
    ...
    private QuoteFeedData getHistoricalQuotes(Security security, boolean collectRawResponse, int count)
    {
        ...
        try
        {
            ...
            long startDateEpochSecond = startDate.atStartOfDay().toEpochSecond(ZoneOffset.UTC);
            
            long endDateEpochSecond = LocalDateTime.now().toEpochSecond(ZoneOffset.UTC);
            
            @SuppressWarnings("nls")
            WebAccess webaccess = new WebAccess("wapi.bvb.ro", "api/history")
                            .addParameter("symbol", security.getTickerSymbol())
                            .addParameter("dt", "DAILY")
                            .addParameter("p", "day")
                            .addParameter("ajust", "1")
                            .addParameter("from", ""+ startDateEpochSecond)
                            .addParameter("to", "" + endDateEpochSecond)
                            .addHeader("Referer", "https://www.bvb.ro/")
                            .addHeader("Host", "wapi.bvb.ro");
