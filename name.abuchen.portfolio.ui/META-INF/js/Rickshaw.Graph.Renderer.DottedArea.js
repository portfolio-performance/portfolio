Rickshaw.namespace('Rickshaw.Graph.Renderer.DottedArea');

Rickshaw.Graph.Renderer.DottedArea = Rickshaw.Class.create(
		Rickshaw.Graph.Renderer, {
			...
			render : function(args) {
				...
				nodes.append("svg:path").attr("d", this.seriesStrokeFactory())
						.attr("class", 'line');
			...
			_styleSeries : function(series) {
				...
				d3.select(series.path).select('.area').attr('fill',
						series.color);
				...
				if (series.stroke) {
					d3.select(series.path).select('.line').attr('fill', 'none')
							.attr(
									'stroke',
									series.stroke
											|| d3.interpolateRgb(series.color,
													'black')(0.125)).attr(
