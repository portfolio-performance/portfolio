...
public class ColorArchitectureDebugView extends AbstractFinanceView
{
    ...
    private void createInfoBlock(Composite parent)
    {
        ...
        text.setText("Diese View zeigt aktuelle Farbwerte und trennt bei instrumentierten Dom├ñnen zwischen CSS-backed und fallback. " //$NON-NLS-1$
        ...
        launch.setText("VM-Argument f├╝r Eclipse Debug-Run: -Dname.abuchen.portfolio.debug=yes"); //$NON-NLS-1$
    ...
    private void createValueColorSchemeSection(Composite parent)
    {
        ...
        for (var scheme : ValueColorScheme.getAvailableSchemes())
        {
            ...
            String note = scheme == ValueColorScheme.current() ? "aktives Scheme" : "verf├╝gbares Scheme"; //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private SecuritiesChartSnapshot buildSecuritiesChartSnapshot(Composite parent)
    {
        ...
        {
            // Debug-View soll bei Teilfehlern trotzdem ├Âffnen
        }
    ...
    private String sampleGradientText(ColorGradientDefinitions.Definition definition)
    {
        ...
        return toHex(start) + " ÔÇª " + toHex(mid) + " ÔÇª " + toHex(end); //$NON-NLS-1$ //$NON-NLS-2$
    }
