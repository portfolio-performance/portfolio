...
public class FIREWidget extends WidgetDelegate<FIREWidget.FIREData>
{
    ...
    private abstract static class MoneyWidgetConfig implements WidgetConfig
    {
        ...
        public void menuAboutToShow(IMenuManager manager)
        {
            ...
            manager.add(new SimpleAction(label + "...", a -> { //$NON-NLS-1$
                var initial = money != null ? Values.Amount.format(money.getAmount()) : ""; //$NON-NLS-1$
    ...
    public Composite createControl(Composite parent, DashboardResources resources)
    {
        ...
        currentValueLbl.setText(
                        MessageFormat.format(Messages.LabelColonSeparated, Messages.LabelFIRECurrentNetWorth, "")); //$NON-NLS-1$
        ...
        currentValueLabel.setText(""); //$NON-NLS-1$
        ...
        timeToFireLabel.setText(""); //$NON-NLS-1$
    ...
    public Supplier<FIREData> getUpdateTask()
    {
        return () -> {
            ...
            // Calculate current portfolio value using the configured data
            // series
            ...
            if (dataSeries != null)
            {
                ...
                var interval = Interval.of(now.minusYears(1), now);
                var index = getDashboardData().calculate(dataSeries, interval);
            ...
            var monthlySavings = get(FIREMonthlySavingsConfig.class).getMonthlySavings();
            var userReturns = get(FIREReturnsConfig.class).getReturns();
            ...
            if (fireNumber != null && currentValue != null && monthlySavings != null && !Double.isNaN(twror))
            {
                ...
                // avoid setting a misleading capped date
                if (timeToFire > 0 && timeToFire < MAX_TIME_TO_FIRE_YEARS - EPSILON_MONTHS / 12.0)
                {
                    long daysToAdd = Math.round(timeToFire * 365.25);
                    targetDate = now.plusDays(daysToAdd);
                }
            }
    ...
    static double calculateTimeToFIRE(long currentValue, long fireNumber, long monthlySavings, double annualReturn)
    {
    ...
    {
        if (data.currentValue() == null || data.fireNumber() == null || data.monthlySavings() == null
                        || Double.isNaN(data.twror())
                        || data.timeToFire() < MAX_TIME_TO_FIRE_YEARS - EPSILON_MONTHS / 12.0)
            return false;

        long monthlySavings = data.monthlySavings().getAmount();
        if (monthlySavings <= 0)
            return false;

        double monthlyReturn = Math.pow(1.0 + data.twror(), 1.0 / 12.0) - 1.0;
        if (monthlyReturn <= 0)
            return false;

        double maxMonths = MAX_TIME_TO_FIRE_YEARS * 12.0;
        double powerTerm = Math.pow(1 + monthlyReturn, maxMonths);
        double projectedValue = data.currentValue().getAmount() * powerTerm
                        + monthlySavings * ((powerTerm - 1) / monthlyReturn);

        return projectedValue < data.fireNumber().getAmount();
    }
    ...
    public void update(FIREData data)
    {
        ...
        {
            currentValueLabel.setText(Values.MoneyShort.format(data.currentValue(), currency));
        }
        else
        {
            currentValueLabel.setText("-"); //$NON-NLS-1$
        ...
        Money userFireNumber = get(FIRENumberConfig.class).getFireNumber();
        if (userFireNumber != null)
        {
            fireNumberLabel.setText(Values.MoneyShort.format(userFireNumber, currency));
            fireNumberInput.setText(Values.Amount.format(userFireNumber.getAmount()));
        }
        else
        {
            fireNumberLabel.setText(Messages.LabelFIREClickToSet);
        ...
        if (userMonthlySavings != null)
        {
            ...
            monthlySavingsInput.setText(Values.Amount.format(userMonthlySavings.getAmount()));
        }
        else
        {
            monthlySavingsLabel.setText(Messages.LabelFIREClickToSet);
        ...
        if (userReturns != null)
        {
            ...
            twrorInput.setText(Values.Percent.format(userReturns));
        }
        else
        {
            twrorLabel.setText(Messages.LabelFIREClickToSet);
        ...
        if (data.fireNumber() == null || Double.isNaN(data.timeToFire()))
        {
            timeToFireLabel.setText("-"); //$NON-NLS-1$
        ...
        else if (data.timeToFire() > 0 && data.timeToFire() < 100)
        {
            timeToFireLabel.setText(MessageFormat.format(Messages.LabelMetricYearsFormatter, data.timeToFire()));
            timeToFireLabel.setTextColor(Colors.theme().defaultForeground());

            if (data.targetDate() != null)
            {
                DateTimeFormatter formatter = DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM);
                targetDateLabel.setText(data.targetDate().format(formatter));
                targetDateLabel.setTextColor(Colors.theme().defaultForeground());
            }
            else
            {
