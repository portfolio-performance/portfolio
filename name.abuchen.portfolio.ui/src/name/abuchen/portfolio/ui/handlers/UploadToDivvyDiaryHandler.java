...
{
    record Selection(long divvyDiaryPortfolioId, Portfolio portfolio)
    {
    }
                    ...
                    @org.eclipse.e4.core.di.annotations.Optional @Preference(UIConstants.Preferences.ENABLE_EXPERIMENTAL_FEATURES) boolean enableExperimentalFeatures)
    {
        ...
        MenuHelper.getActiveClientInput(part).ifPresent(clientInput -> {
            ...
            retrieveAndPickPortfolio(shell, clientInput.getClient(), uploader).ifPresent(
                            selection -> uploadPortfolio(clientInput, uploader, selection.divvyDiaryPortfolioId(),
                                            selection.portfolio, enableExperimentalFeatures));
        });

    }

    private Optional<Selection> retrieveAndPickPortfolio(Shell shell, Client client, DivvyDiaryUploader uploader)
    {
        ...
        ppPortfolios.addAll(client.getPortfolios());
        ...
        if (dialog.open() == Window.OK)
        {
            ...
            return Optional.of(new Selection(selected.id(), portfolio));
        }
    ...
    private void uploadPortfolio(ClientInput clientInput, DivvyDiaryUploader uploader, Long portfolioId,
                    Portfolio portfolio, boolean enableExperimentalFeatures)
    {
        new AbstractClientJob(clientInput.getClient(), Messages.DivvyDiaryMsgUploading)
        {
            ...
            protected IStatus run(IProgressMonitor monitor)
            {
                try
                {
                    ...
                    if (portfolio != null)
                        client = new PortfolioClientFilter(portfolio).filter(client);

                    uploader.upload(client, converter, portfolioId, enableExperimentalFeatures);
