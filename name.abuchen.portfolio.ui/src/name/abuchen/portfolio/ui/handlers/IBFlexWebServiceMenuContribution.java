...
public class IBFlexWebServiceMenuContribution
{
    ...
    public void init(MApplication app)
    {
        ...
        if (command == null)
            throw new IllegalStateException("command " + UIConstants.Command.IMPORT_IB_FLEX_WEBSERVICE + " not found"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (queryIdParameter == null)
            throw new IllegalStateException(
                            "parameter " + UIConstants.Parameter.IBFLEX_QUERY_ID + " not found on command"); //$NON-NLS-1$ //$NON-NLS-2$
    }
