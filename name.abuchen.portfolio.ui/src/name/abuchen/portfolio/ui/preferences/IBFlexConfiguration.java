...
public class IBFlexConfiguration
{
    ...
    public record Credential(String queryId, String token, String name)
    {
        ...
        public String label()
        {
            return name == null ? queryId : name + " (" + queryId + ")"; //$NON-NLS-1$ //$NON-NLS-2$
        }
    }
    ...
    private static final String PROPERTY_LAST_IMPORT_DATES = "ibflex-last-import-dates"; //$NON-NLS-1$

    private IBFlexConfiguration()
    {
    }
    ...
    public static List<Cutoff> getLastImportDates(Client client)
    {
        ...
        var dates = loadLastImportDates(client);
        return dates.entrySet().stream().map(e -> new Cutoff(e.getKey(), parseDate(e.getValue())))
                        .filter(c -> c.date() != null).toList();
    }
