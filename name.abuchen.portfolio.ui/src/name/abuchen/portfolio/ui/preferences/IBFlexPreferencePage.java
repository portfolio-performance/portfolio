...
public class IBFlexPreferencePage extends PreferencePage
{
    ...
    private static final class EditCredentialDialog extends Dialog
    {
        ...
        private EditCredentialDialog(Shell parentShell, List<Credential> existingCredentials, Credential originalCredential)
        {
            super(parentShell);
        ...
        private boolean isDuplicateQueryId(String queryId)
        {
            return existingCredentials.stream()
                            .filter(credential -> credential != originalCredential)
    ...
    protected Control createContents(Composite parent)
    {
        ...
        link.addSelectionListener(SelectionListener.widgetSelectedAdapter(
                        event -> DesktopAPI.browse("https://www.interactivebrokers.com/campus/ibkr-api-page/flex-web-service/"))); //$NON-NLS-1$

        credentials = new ArrayList<>(IBFlexConfigurationDialog.getCredentials());
        cutoffs = client.map(activeClient -> new ArrayList<>(IBFlexConfigurationDialog.getLastImportDates(activeClient)))
        ...
        editCredentialButton.addSelectionListener(SelectionListener.widgetSelectedAdapter(e -> editSelectedCredential()));
        ...
        removeCredentialButton.addSelectionListener(SelectionListener.widgetSelectedAdapter(e -> removeSelectedCredentials()));
    ...
    public boolean performOk()
    {
        getPreferenceStore().setValue(UIConstants.Preferences.IBFLEX_CREDENTIALS,
                        IBFlexConfigurationDialog.serialize(credentials));
        client.ifPresent(activeClient -> IBFlexConfigurationDialog.setLastImportDates(activeClient, cutoffs));
        return true;
    }
