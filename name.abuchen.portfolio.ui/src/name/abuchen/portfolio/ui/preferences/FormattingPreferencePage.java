    ...
    protected void createFieldEditors()
    {
        ...
        if (client.isPresent())
        {
            var elements = Arrays.stream(SecurityNameConfig.values())
                            .map(entry -> new String[] { entry.toString(), entry.name() }).toArray(String[][]::new);

            var fieldEditor = new ComboFieldEditor(ClientProperties.Keys.SECURITY_NAME_CONFIG, "Prefix name with...",
                            elements, getFieldEditorParent());
