...
{
    public static final String DEFAULT_SCHEME = "greenred"; //$NON-NLS-1$

    private static final List<ValueColorScheme> schemes;
    private static volatile ValueColorScheme currentScheme;

    static
    {
        // initialize value color scheme with values in case CSS cannot be read

        var greenRed = new ValueColorScheme(DEFAULT_SCHEME);
        greenRed.setPositiveForeground(Colors.DARK_GREEN.getRGBA());
        greenRed.setNegativeForeground(Colors.DARK_RED.getRGBA());
        greenRed.setUpArrow("greenred/light/up_arrow.svg"); //$NON-NLS-1$
        greenRed.setDownArrow("greenred/light/down_arrow.svg"); //$NON-NLS-1$
        ...
        blueOrange.setUpArrow("blueorange/light/up_arrow.svg"); //$NON-NLS-1$
        blueOrange.setDownArrow("blueorange/light/down_arrow.svg"); //$NON-NLS-1$

        schemes = List.of(greenRed, blueOrange);
        currentScheme = greenRed;
    }
